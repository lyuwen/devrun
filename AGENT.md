...
* **Inter-stage parameter passing:** Downstream workflow stages can reference params from completed upstream stages via two mechanisms:
  * **Cross-stage references** (`${stages:stage_name,param_key}` in YAML): registered as a custom OmegaConf resolver that emits a `<<STAGE_REF:stage:param>>` sentinel at config-load time. `WorkflowRunner._resolve_stage_params()` replaces sentinels with actual values at submit time. Supports string concatenation (e.g. `"${stages:inference,output_dir}/predictions.jsonl"`). When a sentinel spans the full value, the original type (int, dict, etc.) is preserved; when embedded in a string, the referenced value must be a scalar.
  * **Auto-forward:** Before submitting a stage, params from dependency stages' `resolved_params` are auto-forwarded for any key not already present in the stage's explicit params. First dependency in `depends_on` order wins. Tracked in `stages_state[name]["resolved_params"]` and logged at DEBUG level.
  * **Resolution order** per stage at submit time: (1) stage's explicit params from config, (2) auto-forward missing keys from dependency stages, (3) resolve `<<STAGE_REF:...>>` sentinels to actual values.
  * **`--from-job` integration:** When a stage is skipped via `--start-after` and `--from-job` provides a job record, the skipped stage's `resolved_params` are populated from the job's stored params in the DB, enabling downstream cross-stage refs to resolve correctly.
...
- **775 tests passing**, **10 skipped** (infrastructure-dependent: require real SSH/Slurm connectivity)
